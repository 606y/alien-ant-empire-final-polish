...
(function(root){
  ...
  class World {
    ...
    attackTarget(s,a){
      ...
      // Bounded local connectivity mirrors eligibility, without simulation path calls.
      const reachable=new Set([a.k]);let frontier=[a.k];for(let i=0;i<(acid?3:1);i++){const next=[];for(const k of frontier)for(const n of E.neighbors(s,k))if(!reachable.has(n)){reachable.add(n);next.push(n);}frontier=next;}
      return s.ants.filter(b=>b.faction!==a.faction&&b.hp>0&&E.distance(a,b)<range&&reachable.has(b.k)).sort((b,c)=>E.distance(a,b)-E.distance(a,c))[0]||null;
    }
    queenFacing(owner,x,y){const old=this.facingCache.get(owner),dx=old?x-old.x:0,dy=old?y-old.y:0,angle=Math.hypot(dx,dy)>.0001?Math.atan2(dy,dx):old?.angle??0;this.facingCache.set(owner,{x,y,angle});return angle;}
    facing(s,a){
      const old=this.facingCache.get(a),target=this.attackTarget(s,a);let dx=0,dy=0;
      if(target){dx=target.x-a.x;dy=target.y-a.y;}
      else if(old&&E.isSurface(s,a.k)===old.surface){dx=a.x-old.x;dy=a.y-old.y;}
      if(Math.hypot(dx,dy)<.0001&&!target&&a.path?.length){const p=E.xy(a.path[0]);dx=p.x-a.x;dy=p.y-a.y;}
      const angle=Math.hypot(dx,dy)>.0001?Math.atan2(dy,dx):old?.angle??0;
