...
(function(root){
  ...
  const V7_SPRITES=[
    'world/textures/nest_soil_tile','world/textures/surface_ground_tile','world/textures/tunnel_rim_overlay',
    'world/effects/selection_ring','world/effects/command_move','world/effects/command_attack','world/effects/command_gather','world/effects/command_build','world/effects/command_cross',
  ...
  class World {
    ...
    ant(x,y,size,color,angle,a,queen=false,visualKey=null){
      if(visualKey&&this.sprite(visualKey,x,y,size*2,size*2,angle))return;
    ...
    draw(s,{selected=[],target=null,digging=false,commandMarker=null}={},elapsed=.016){
      ...
      if(this.view==='nest'){
        c.lineCap='round';
        for(const [color,width]of [['#6a6848',.87],['#1a120d',.69]])for(const tile of s.cells){
      ...
      for(const a of ants){if(!visibleCell(a.k))continue;const elevation=E.cell(s,a.k)?.elevation||0,p=this.screen(a.x+(noise(a.id,1)-.5)*.5,a.y+(noise(a.id,2)-.5)*.42,elevation);const next=a.path.length?E.xy(a.path[0]):null,angle=next?Math.atan2(next.y-a.y,next.x-a.x):noise(a.id,7)*6;
        if(selected.includes(a.id)){const ring=this.v7Images['world/effects/selection_ring'],radius=Math.max(12,scale*.3);if(ring?.naturalWidth)c.drawImage(ring,p.x-radius*1.25,p.y-radius*1.25,radius*2.5,radius*2.5);else{c.strokeStyle='#ddecaa';c.lineWidth=1;c.beginPath();c.arc(p.x,p.y,Math.max(12,scale*.3),0,7);c.stroke();}}
