...
test('new-game film preserves the existing three slides and has a skip and failure path',()=>{
 ...
 assert.match(html,/id="cinematicVideo" playsinline/);assert.match(html,/intro_cinematic_v6_poster\.jpg/);assert.match(html,/intro_cinematic_v6_10s\.mp4/);
 assert.match(app,/function startNewGame/);assert.match(app,/function finishCinematic/);assert.match(app,/showIntro\(\);audio\.resumeAfterCinematic\(\)/);assert.match(app,/skipCinematic/);assert.match(app,/console\.warn\('V6\.1 cinematic unavailable/);
