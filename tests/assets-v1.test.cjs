...
const reserved=[
  ...
  'assets/intro/intro_01_forest_stirring.png','assets/intro/intro_02_ant_emerges.png','assets/intro/intro_03_winged_rise.png','assets/intro/intro_04_empire_reveal.png','assets/intro/intro_05_title_card.png',
...
test('Assets v1 reserves every requested path and directory without requesting absent files',async()=>{
  ...
  assert.equal(entries.length,reserved.length);for(const wanted of reserved){const entry=entries.find(item=>item.src===wanted);assert.ok(entry,wanted);assert.equal(entry.enabled,wanted==='assets/menu/menu_bg_main.png',wanted);}
  assert.equal(context.window.AntAssets.state('menuBgMain'),'ready');assert.ok(Object.entries(context.window.AntAssets.report()).filter(([key])=>key!=='menuBgMain').every(([,state])=>state==='fallback'));
});
