...
const reserved=[
  ...
  'assets/menu/animated/flyer_guard_01.png','assets/menu/animated/flyer_heavy_01.png','assets/menu/animated/flyer_scout_01.png',
...
test('Assets v1 reserves every requested path and loads safe image slots',async()=>{
  for(const dir of ['assets/menu','assets/menu/animated','assets/intro','assets/logo','assets/audio/bgm','assets/audio/sfx','assets/docs'])assert.equal(fs.statSync(path.join(root,dir)).isDirectory(),true,dir);
...
test('formal menu, intro, logo and audio slots are wired before procedural fallbacks',()=>{
  ...
  for(const key of ['menuBgMain','menuFogOverlay','menuForegroundOverlay','flyerGuard01','flyerHeavy01','flyerScout01'])assert.match(menu+read('assets/manifest.js'),new RegExp(key));
