...
test('final presentation includes a procedural menu, intro, audio and no remote asset dependency',()=>{
  ...
  assert.match(menu,/class MenuArt/);assert.match(menu,/function mound/);assert.match(menu,/function forest/);assert.match(audio,/class AntAudio/);assert.match(audio,/AudioContext/);assert.match(audio,/setScene/);assert.match(audio,/musicTick/);assert.match(audio,/nest.*surface.*combat/);
  assert.match(css,/\.main-menu/);assert.match(css,/\.intro/);assert.doesNotMatch(html,/<(?:script|link|img)[^>]+(?:src|href)="https?:\/\//i);assert.doesNotMatch(css,/url\(["']?https?:\/\//i);assert.doesNotMatch(menu+audio,/https?:\/\//);
});
